...
export default function SettingsPage() {
  ...
  const [downloadAlertThreshold, setDownloadAlertThreshold] = useState(() => localStorage.getItem('tw_download_alert_threshold') || '')
  const [uploadAlertThreshold, setUploadAlertThreshold] = useState(() => localStorage.getItem('tw_upload_alert_threshold') || '')
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('tw_download_alert_threshold', downloadAlertThreshold)
    localStorage.setItem('tw_upload_alert_threshold', uploadAlertThreshold)
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="space-y-3">
        <Section title="Language">
          <button onClick={toggleLang} className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-gray-300 hover:bg-white/10">
        ...
        <Section title="Download Alert Threshold">
          <div className="flex gap-3">
            <input type="number" placeholder="e.g. 20" value={downloadAlertThreshold}
              onChange={e => setDownloadAlertThreshold(e.target.value)}
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-gray-300 outline-none"
            />
            <span className="flex items-center text-sm text-gray-500">Mbps</span>
          </div>
          <p className="text-[11px] text-gray-700 mt-1.5">Alert when download speed drops below this threshold (0 = off)</p>
        </Section>

        <Section title="Upload Alert Threshold">
          <div className="flex gap-3">
            <input type="number" placeholder="e.g. 10" value={uploadAlertThreshold}
              onChange={e => setUploadAlertThreshold(e.target.value)}
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-gray-300 outline-none"
            />
            <span className="flex items-center text-sm text-gray-500">Mbps</span>
          </div>
          <p className="text-[11px] text-gray-700 mt-1.5">Alert when upload speed drops below this threshold (0 = off)</p>
        </Section>
        ...
        <Section title="Export">
          <button onClick={handleExport} className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-gray-400 hover:bg-white/10">
